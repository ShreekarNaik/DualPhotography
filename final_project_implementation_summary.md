...
### The New Algorithm: "Bounded Hierarchical Scan"

Update your report with this section. This satisfies the "Optimal Control" criteria by minimizing the cost function $J$ (measurements) based on state observation (ROI).

#### 1. Phase 1: System Identification (The Coarse Scan)

Instead of projecting the full sequence, we start with the **Most Significant Bits (MSBs)**.

- **Action:** Project the top $K$ bits of Gray Codes (e.g., $K=3$).
- **Resolution:** This divides the projector into an $8 \times 8$ grid of large blocks.
- **Feedback:** Capture images and threshold them (using the Confidence Mask).
- **State Estimate:** Identify which of the 64 blocks contain _any_ active pixels.

#### 2. Phase 2: ROI Estimation (The Controller)

Calculate the **Active Bounding Box** from the active blocks.

- Let $U_{min}, U_{max}$ be the range of active horizontal blocks.
- Let $V_{min}, V_{max}$ be the range of active vertical blocks.
- **Calculate Required Precision:**
  $$W_{active} = (U_{max} - U_{min} + 1) \times \text{BlockSize}$$
  $$Bits_{needed} = \lceil \log_2(W_{active}) \rceil$$

#### 3. Phase 3: Adaptive Refinement

- **Case A (Dense/Worst Case):**
  If the object covers the whole screen ($W_{active} \approx W_{proj}$), then $Bits_{needed} \approx \text{All Bits}$.

  - **Action:** Continue projecting the remaining standard Gray Codes.
  - **Cost:** Exactly the same as the Video Algorithm ($O(\log N)$). **Safety Guarantee Met.**

- **Case B (Sparse/Best Case):**
  If the object is small ($W_{active} \ll W_{proj}$), we effectively "Zoom In" the Gray Codes.
  - **Action:** Generate **Shifted Gray Codes** that map the range $[0, 2^{Bits_{needed}}-1]$ to the ROI $[U_{min}, U_{max}]$.
  - **Project:** Only the $Bits_{needed}$ patterns.
  - **Cost:** $K_{MSB} + Bits_{needed}$. This is strictly less than $TotalBits$.
